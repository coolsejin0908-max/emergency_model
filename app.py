...
# ------------------------------
# 0. M 좌표 (TM) -> 위도/경도 변환 (예시 함수)
# ------------------------------
def tm_to_wgs84(tm_x, tm_y):
    # 실제 변환을 원하면 pyproj 라이브러리 사용 권장
    base_tm_x = 200000
    base_tm_y = 500000
    base_lat = 37.5665
    base_lon = 126.9780
...
def load_hospital_data_with_m():
    # 샘플 데이터 (실제로는 CSV 또는 API 로드)
    data = {
        "name": ["서울대병원", "세브란스병원", "삼성서울병원", "서울아산병원", "강남세브란스",
                 "한양대병원", "중앙대병원", "경희대병원", "가톨릭여의도", "인제상계"],
        "tm_x": [198000, 195500, 203000, 201000, 206000, 199500, 197000, 204000, 202000, 208000],
        "tm_y": [542000, 541500, 540000, 540500, 539000, 541000, 540200, 542500, 540800, 539500],
        "beds": [120, 90, 110, 95, 70, 65, 80, 75, 85, 60],
        "doctors": [150, 120, 140, 130, 90, 85, 100, 95, 110, 80],
        "specialties": ["외상,심장,신경", "외상,심장", "외상,심장,정형", "심장,신경,소아", "외상,정형",
                        "외상,소아", "심장,신경", "외상,정형,신경", "심장,외상", "정형,소아"],
    }
...
if mode == "간편 모드 (질문)":
    ...
    hospital_size_pref = st.sidebar.selectbox("선호하는 병원 규모", ["모든 규모", "작은 병원", "중간 병원", "큰 병원"])
    expert_beds = 150
    expert_staff = 80
else:
...
nearby = hospitals_df[hospitals_df["distance"] <= radius_km].copy().reset_index(drop=True)

if nearby.empty:
    st.warning(f"반경 {radius_km}km 내 병원이 없습니다.")
    st.stop()

# 병원 규모 필터링
if "작은 병원" in hospital_size_pref and "모든" not in hospital_size_pref:
...
if nearby.empty:
    st.warning("선택한 규모의 병원이 반경 내에 없습니다. 필터를 완화해보세요.")
    st.stop()
...
nearby["specialty_match"] = [match_specialty(row, required_specialty) for _, row in nearby.iterrows()]

# 혼잡도 계산 (병상 대비 의사 수 기준)
nearby["congestion"] = [
    "혼잡" if row["doctors"]/row["beds"] < 1.2 else ("보통" if row["doctors"]/row["beds"] < 2.0 else "여유")
...
# 추천 점수
congestion_score_map = {"여유":3, "보통":1, "혼잡":0}
nearby["score"] = (
    nearby["specialty_match"] * 10 
    + 1 / (nearby["distance"] + 0.1) 
    + [congestion_score_map[c] for c in nearby["congestion"]]
)
...
if show_heatmap:
    heat_data = [[row["lat"], row["lon"], 1 if row["congestion"]=="혼잡" else 0.5] for _, row in nearby.iterrows()]
...
display_df = nearby[["name", "distance", "beds", "doctors", "congestion", "specialties", "score"]].copy()
display_df["distance"] = display_df["distance"].map(lambda x: f"{x:.1f} km")
...
if mode == "전문가 모드 (슬라이더)":
    ...
    with col1:
        expert_beds2 = st.number_input("병상 수 (전문가 예측용)", 20, 500, expert_beds)
    ...
    predicted_avail = min(0.95, (expert_doctors + expert_nurses) / (expert_beds2 * 2.5))
...
st.caption("※ M 좌표 변환은 샘플 함수이며, 실제 데이터 사용 시 pyproj 라이브러리로 정확한 변환 필요")
